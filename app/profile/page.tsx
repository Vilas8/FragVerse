...
export default async function Profile() {
  try {
    ...
    const [publicUserResult, tournamentsResult, commentsResult, matchesResult, statisticsResult] = await Promise.allSettled([
                      ...
                      <CyberCard key={match.id} variant="purple" hover>
                        <CyberCardHeader>
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <CyberCardTitle className="text-lg mb-2">
                                {match.tournaments?.name || 'Tournament'} - Round {match.round}
                              </CyberCardTitle>
              ...
              <CyberCardContent>
                {statistics ? (
                  ...
                  <div className="text-center py-8">
                    <TrendingUp className="w-12 h-12 text-purple-500/30 mx-auto mb-3" />
                    <p className="text-cyan-100/60">No statistics available yet</p>
                  </div>
                )}
              </CyberCardContent>
            </CyberCard>
